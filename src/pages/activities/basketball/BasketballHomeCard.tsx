...
export default function BasketballProgress() {
  ...
  return (
    <Card
      title={`🏀 ${latestLocation?.name ? ` ${latestLocation.name}` : ""}`}
      subtitle={
      ...
      {latest && latestLocation?.lat != null && latestLocation?.lon != null ? (
        <>
          ...
          <ul className="text-gray-300 text-sm space-y-1">
            <li>
              <strong>Duration:</strong> {latest.data.duration_min} min
            </li>
            <li>
              <strong>Games:</strong> {latest.data.games}
            </li>
            <li>
