...
const ACTIVITY_ID = "f7a49b99-ab6f-4bc7-982f-ab1b9bd0b187";
...
export default function Basketball() {
  ...
  useEffect(() => {
    async function fetchAllLogs() {
      ...
      if (error) {
        ...
        setError("Failed to load Basketball sessions. Please refresh.");
        setLogs([]);
      } else if (data) {
        setLogs(data);
      }
    ...
    async function fetchLogForDate() {
      ...
      if (data) {
        setForm({
          ...
          games: data.data?.games || 0,
          players: data.data?.players || 0,
          notes: data.data?.notes || "",
        });
      } else {
        setForm({ location: "", duration_min: 0, games: 0, players: 0, notes: "" });
      }
    }
    fetchLogForDate();
  }, [datetime, user]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert("Basketball session logged!");
  ...
  const groupedLogsByLocation = groupLogsByLocation(
    filteredLogs,
    locations,
    (log) => (log.data.games as number) ?? 0,
  );
  ...
  return (
    <div className="flex flex-col md:flex-row gap-8 p-6">
      ...
      <div className="flex-1 space-y-6">
        <h1 className="text-3xl font-bold">Log a Basketball Session</h1>
        <form
        ...
        >
          ...
          <div>
            <label className="block mb-1">Games</label>
            <input
              type="number"
              className="w-full p-2 rounded bg-slate-700 text-white"
              value={form.games}
              onChange={(e) =>
                setForm({ ...form, games: parseInt(e.target.value || "0") })
              }
            />
          </div>
        ...
        <div className="bg-slate-800 rounded-xl p-4 max-h-[400px] overflow-y-auto shadow-md">
          <h2 className="text-xl font-semibold mb-2">Basketball Session History</h2>
          {loading ? (
      ...
      <div className="md:w-1/2 space-y-6">
        <h1 className="text-3xl font-bold">Basketball Statistics</h1>
        <StatisticsSection
          logs={logs}
          ...
          computeStats={(filtered) => {
            ...
            const { totalMinutes, totalGames, totalPlayers } = filtered.reduce(
              (acc, l) => {
                acc.totalMinutes += l.data?.duration_min ?? 0;
                acc.totalGames += l.data?.games ?? 0;
                acc.totalPlayers += l.data?.players ?? 0;
                return acc;
              },
              { totalMinutes: 0, totalGames: 0, totalPlayers: 0 },
            );
