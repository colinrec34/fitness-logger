...
export default function SurfProgress() {
  ...
  return (
    <Card
      title={`🏄 ${latestLocation?.name ?? "Last Surf Session"}`}
      subtitle={
      ...
      {latest && latestLocation?.lat != null && latestLocation?.lon != null ? (
        <>
          ...
          <ul className="text-gray-300 text-sm space-y-1">
            <li>
              <strong>Board:</strong> {latest.data.board}
            </li>
            <li>
              <strong>Duration:</strong> {latest.data.duration} min
            </li>
            <li>
              <strong>Waves Caught:</strong> {latest.data.waves}
            </li>
            {latest.data.height && (
              <li>
                <strong>Wave Height:</strong> {latest.data.height}
              </li>
            )}
          </ul>
        </>
